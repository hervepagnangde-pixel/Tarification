...
def _hill_estimates(sorted_desc, k_max=None):
    ...
    for k in range(1, k_max+1):
        log_r = np.log(sorted_desc[:k] / sorted_desc[k])
        h = k / np.sum(log_r) if np.sum(log_r) > 0 else np.nan
...
def _gertensgarbe_k(ks, hills):
    valid = ~np.isnan(hills); h = hills[valid]; k = ks[valid]
    if len(h) < 10: return k[len(k)//2]
    n = len(h); s_prog = np.zeros(n); s_reg = np.zeros(n)
    for i in range(1, n):
...
def _fit_severity(exceedances, threshold):
    ...
    try:
        y = x - threshold; xi, loc, beta = stats.genpareto.fit(y, floc=0)
...
def _threshold_table(data, thresholds_pct):
    ...
    for pct in thresholds_pct:
        u = np.percentile(data, pct); exc = data[data > u]; n_exc = len(exc)
        if n_exc < 5:
        ...
        try:
            cdf_v = np.sort(1-(np.sort(exc)/u)**(-alpha_h)); nn = len(cdf_v); i_a = np.arange(1, nn+1)
            ad_stat = -nn - np.mean((2*i_a-1)*(np.log(np.clip(cdf_v,1e-10,1-1e-10))+np.log(np.clip(1-cdf_v[::-1],1e-10,1-1e-10))))
...
def section_analyse_distributions():
    ...
    if "df_proj" not in st.session_state or "alpha_est" not in st.session_state:
        st.info("Transformez d'abord le triangle."); return
    ...
    if len(sev_data) < 10:
        st.warning(f"Seulement {len(sev_data)} sinistres au-dessus du seuil."); return
    ...
    with tabs_d[0]:
        ...
        with col_h:
            ...
            ax_h.axvline(k_gert, color="#ef4444", ls="--", lw=1.5,
                         label=f"Gertensgarbe k={k_gert} alpha={alpha_gert:.3f}")
            ax_h.axhline(alpha_0, color="#2d8a4e", ls=":", lw=1.2, label=f"alpha actuel={alpha_0:.3f}")
            ax_h.set_xlabel("k"); ax_h.set_ylabel("alpha(k)"); ax_h.set_title("Hill Plot")
            ax_h.legend(fontsize=8); ax_h.grid(alpha=0.3); st.pyplot(fig_h); plt.close()
        with col_m:
            ...
            ax_m.legend(fontsize=8); ax_m.grid(alpha=0.3); st.pyplot(fig_m); plt.close()
        rows_s = _threshold_table(all_sev, [50, 60, 70, 75, 80, 85, 90, 95])
        ...
        if best_row:
            st.success("Recommandé : " + str(best_row["Seuil %"]) + " = " + str(best_row["Seuil MAD"]) + " MAD — alpha=" + str(best_row["Alpha Hill"]))
            if st.button("Appliquer", key="btn_apply_seuil"):
                st.session_state["seuil_est"] = float(best_row["Seuil MAD"].replace(",","").replace(" ",""))
                st.session_state["alpha_est"] = float(best_row["Alpha Hill"]); st.rerun()

    with tabs_d[1]:
        fits_sev = _fit_severity(sev_data, seuil_0)
        if fits_sev:
            rows_fit = []
            for n, f in fits_sev.items():
                if n == "Pareto":    params = "alpha=" + str(round(f["alpha"],4))
                elif n == "Log-Normale": params = "mu=" + str(round(f["mu"],3)) + " sigma=" + str(round(f["sigma"],3))
                else:                params = "xi=" + str(round(f.get("xi",0),4)) + " beta=" + str(round(f.get("beta",0),0))
                rows_fit.append({"Distribution": n, "Paramètres": params,
                    "KS": f"{f['ks']:.4f}", "p-val": f"{f['pval']:.4f}",
                    "Adéquation": "Bon" if f["pval"]>0.05 else "Acceptable" if f["pval"]>0.01 else "Rejeté"})
            st.dataframe(pd.DataFrame(rows_fit), use_container_width=True)
        ...
        for nom, f in fits_sev.items():
            try:
                ...
                axes_c[0].plot(x_s, y, "--", color=col, lw=1.8, label=nom+" p="+str(round(f["pval"],3)))
            except: pass
        axes_c[0].set_xlabel("MAD"); axes_c[0].set_ylabel("F(x)"); axes_c[0].set_title("CDF Sévérité")
        axes_c[0].legend(fontsize=8); axes_c[0].grid(alpha=0.3)
    ...
    with tabs_d[2]:
        ...
        if fits_freq:
            ...
            for nom, f in fits_freq.items():
                if "note" in f:
                    rows_fr.append({"Distribution":nom,"Paramètres":f["note"],"KS":"—","p-val":"—","Adéquation":"—"})
                else:
                    params_f = "lambda="+str(round(f["lambda"],3)) if nom=="Poisson" else "r="+str(round(f["r"],3))+" p="+str(round(f["p"],4))
                    rows_fr.append({"Distribution":nom,"Paramètres":params_f,
                        "KS":f"{f['ks']:.4f}","p-val":f"{f['pval']:.4f}",
                        "Adéquation":"Bon" if f["pval"]>0.05 else "Acceptable" if f["pval"]>0.01 else "Rejeté"})
        ...
        if "Poisson" in fits_freq and "pval" in fits_freq["Poisson"]:
            ax_fr.plot(x_r, sp_stats.poisson.pmf(x_r, fits_freq["Poisson"]["lambda"]),
                       "r--o", ms=5, lw=1.5, label="Poisson(lambda="+str(round(fits_freq["Poisson"]["lambda"],2))+")")
        if "BN" in fits_freq and "r" in fits_freq["BN"]:
            f_nb = fits_freq["BN"]
            ax_fr.plot(x_r, sp_stats.nbinom.pmf(x_r, f_nb["r"], f_nb["p"]),
                       "b--s", ms=5, lw=1.5, label="BN(r="+str(round(f_nb["r"],2))+")")
        ax_fr.set_xlabel("Sinistres/an"); ax_fr.set_title("Fréquence annuelle")
        ax_fr.legend(); ax_fr.grid(alpha=0.3); st.pyplot(fig_fr); plt.close()
        disp_ratio = float(np.var(freq_data)/max(np.mean(freq_data),0.01))
        label_disp = "surdispersion -> BN pertinente" if disp_ratio > 1.2 else "equipdispersion -> Poisson adapté"
        st.info("Indice de dispersion = " + str(round(disp_ratio,2)) + " (" + label_disp + ")")
    ...
    with tabs_d[3]:
        ...
        if len(exc_m) >= 5:
            fig_mn, ax_mn = plt.subplots(figsize=(8,3)); xs_m = np.sort(exc_m)
            ...
            ax_mn.plot(xs_m, np.clip(1-(xs_m/seuil_m)**(-alpha_m),0,1), "r--", lw=1.8,
                       label="Pareto(alpha="+str(round(alpha_m,2))+")")
            ax_mn.set_xlabel("MAD"); ax_mn.set_title("CDF Sévérité — paramètres manuels")
            ax_mn.legend(); ax_mn.grid(alpha=0.3); st.pyplot(fig_mn); plt.close()
    ...
    try:
        fits_s2 = _fit_severity(sev_data, seuil_0); fits_f2 = _fit_frequency(freq_data)
        st.session_state["dist_fit_results"] = {
            "severity":   {k: {kk: float(vv) if isinstance(vv,(int,float,np.floating)) else vv for kk,vv in v.items() if kk!="threshold"} for k,v in fits_s2.items()},
            "frequency":  {k: {kk: float(vv) if isinstance(vv,(int,float,np.floating)) else vv for kk,vv in v.items()} for k,v in fits_f2.items()},
