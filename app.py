        ...
        with st.expander(" Analyse préalable — Détection des années atypiques (obligatoire)", expanded=True):
            ...
            if t_ref:
                ...
                if exc_ann:
                    ...
                    with c1e: card("τ BC avec atypiques",f"{t_av:.4%}",couleur="#e74c3c",icone=None)
                    with c2e: card("τ BC sans atypiques",f"{t_ss:.4%}",couleur="#00b5a5",icone=None)
                    with c3e: card("Écart",f"{abs(t_av-t_ss)*100:.4f} pts",couleur="#0d2b3e",icone=None)
        ...
        with st.expander(" Triangle individuel — 1 ligne = 1 sinistre", expanded=False):
            if "df_liq" in st.session_state:
                ...
                st.markdown("""<div style="background:#f0fff4;border-left:4px solid #2d8a4e;
                    ...
                    <b>Logique :</b> 1⃣ Chaque sinistre est projeté individuellement à l'ultime →
                    2⃣ Charge XL individuelle : <code>Ck = min(max(S'ultime − D, 0), L)</code> →
                    3⃣ Agrégation par UW Year = BC annuel
                    </div>""", unsafe_allow_html=True)
...
with tab4:
    st.header("Simulation fréquence-sévérité")
    ...
    if "alpha_est" not in st.session_state:
        ...
        with st.expander(" Détection automatique du seuil TVE (Hill + MEF + Gertensgarbe)", expanded=True):
            ...
            st.markdown("** Positionnement interactif du seuil**")
...
with tab5:
    ...
    with st.expander("Parametres de filtrage global", expanded=False):
        ...
        if f_mkt is None:
            st.info(" Uploadez un fichier de données marché pour construire la Courbe de référence marché.")
            st.stop()
...
with tab_agent:
    ...
    st.markdown("""<div style="background:linear-gradient(135deg,#0d2b1a,#1a1a1a);border-radius:12px;
        ...
        <div style="color:#ccc;font-size:13px;line-height:1.9">
            ...
            Recherche de structures alternatives comparables
        </div></div>""", unsafe_allow_html=True)
    ...
    if "df_proj" not in st.session_state:
        ...
        with st.spinner(" Pipeline en cours..."):
            ...
            agent = AgentActuarielPython(
                ...
                df_ml              = st.session_state.get("labo_df_ml", st.session_state.get("df_ml")),
                loi_sim            = st.session_state.get("loi_sim", "pareto"),
                mu_ln              = st.session_state.get("mu_ln_input", st.session_state.get("loi_mu")),
                sigma_ln           = st.session_state.get("sigma_ln_input", st.session_state.get("loi_sigma")),
                gpd_xi             = st.session_state.get("gpd_xi_input", st.session_state.get("gpd_xi")),
                gpd_beta           = st.session_state.get("gpd_beta_input", st.session_state.get("gpd_beta")),
            )
            ...
            opt_ag       = AgentOptimisationProgramme(agent.gnpi).explorer(
                              agent.tranches, agent.resultats_bc, agent.resultats_sim,
                              agent.resultats_mkt, objectif="structures_comparables", top_n=8)
...
with tab_full:
    section_header("Assistant d’interprétation actuarielle LLM", "Fichiers bruts → Rapport final — Raisonnement LLM Claude (API requise)", "")
    ...
    <div style="background:linear-gradient(135deg,#0d0d1a,#1a1a1a);border-radius:12px;
        ...
        <div style="color:#3b82f6;font-weight:700;font-size:15px;margin-bottom:10px">
             Assistant d’interprétation actuarielle — LLM Claude (API Anthropic requise)
        </div>
        <div style="display:flex;gap:32px;flex-wrap:wrap">
            <div style="color:#ccc;font-size:13px;line-height:2">
                 Importer les fichiers utiles à l'analyse<br>
                 Comparer les résultats du moteur et de la tarification externe<br>
                 Contrôler la cohérence des méthodes<br>
                 Identifier les points à documenter
            </div>
            <div style="color:#ccc;font-size:13px;line-height:2">
                 Analyser BC, simulation et courbe de référence marché<br>
                 Intégrer les sinistres majeurs et paramètres de simulation<br>
                 Proposer des programmes alternatifs comparables<br>
                 Enrichir la négociation en position de réassureur leader
            </div>
        </div>
        <div style="color:#888;font-size:12px;margin-top:10px">
            Le LLM ne remplace pas la tarification actuarielle : il interprète les résultats fournis et propose des structures comparables pour soutenir la négociation.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("Tarification manuelle externe à comparer", expanded=True):
        st.caption("Renseignez ici les résultats obtenus hors application : R, Excel, SAS ou autre outil actuariel. Ces valeurs servent de référence de comparaison ; le LLM ne doit pas les recalculer ni les modifier.")
        manuel_col1, manuel_col2 = st.columns(2)
        with manuel_col1:
            ...
            params_manuels3 = st.text_area(
                "Paramètres de simulation validés manuellement",
                placeholder="Exemple : loi retenue = GPD ; seuil = 1 800 000 ; xi = 0.27 ; beta = 620 000 ; lambda = 2.4 ; n_sim = 100 000",
                height=110,
                key="params_manuels_externes3",
            )
        with manuel_col2:
            ...
            commentaires_manuels3 = st.text_area(
                "Commentaires actuariels externes",
                placeholder="Exemple : année 2020 exclue, changement portefeuille 2021, forte incertitude sur tranche Cat L2, courbe marché peu crédible car N=38.",
                height=110,
                key="commentaires_manuels_externes3",
            )

    tarification_manuelle3 = "\n".join([
        "TAUX PAR TRANCHE :\n" + (taux_manuels3 or "Non fournis"),
        "PARAMÈTRES DE SIMULATION :\n" + (params_manuels3 or "Non fournis"),
        "SINISTRES MAJEURS :\n" + (majeurs_manuels3 or "Non fournis"),
        "COMMENTAIRES ACTUARIELS :\n" + (commentaires_manuels3 or "Non fournis"),
    ])
    ...
    lancer3 = st.button(" Lancer l’assistant d’interprétation", type="primary",
...
Claude peut proposer les tranches basées sur le contexte fourni ou ajuster les valeurs par défaut.""",
        ...
        },
        {
            ...
            "description": "Lance la simulation avec la loi de sévérité retenue et les paramètres validés. Ne pas imposer Pareto si Lognormale ou GPD est retenue.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "loi"           : {"type": "string", "enum": ["pareto", "lognormale", "gpd"]},
                    "alpha"         : {"type": "number"},
                    "lambda_"       : {"type": "number"},
                    "seuil"         : {"type": "number"},
                    "mu"            : {"type": "number"},
                ...
                "required": ["loi", "lambda_", "seuil", "n_sim", "justification"]
            }
        },
        {
...
Catégories : réassurance, actuariat, cours, finance, Maroc.""",
    ...
    def _executer_simulation_loi_p3(loi, alpha, lambda_, seuil, mu=None, sigma=None, xi=None, beta=None, n_sim=10000, gnpi_v=None):
        """Simulation utilisée par l'assistant LLM avec la loi réellement retenue."""
        if "coeffs" not in st.session_state:
            return {"erreur": "coeffs manquants"}
        prog = st.session_state.get("tranches_p3", tranches_input)
        coeffs = np.asarray(st.session_state["coeffs"], dtype=float)
        coeffs = coeffs[np.isfinite(coeffs)]
        if len(coeffs) == 0:
            coeffs = np.array([1.0])
        loi = str(loi or "pareto").lower()
        lambda_ = float(lambda_ or 0.0)
        seuil = float(seuil or 0.0)
        gnpi_v = float(gnpi_v or gnpi)
        if lambda_ <= 0 or seuil <= 0:
            return {"erreur": "lambda ou seuil invalide"}
        if loi == "pareto":
            alpha = float(alpha or 0.0)
            if alpha <= 0:
                return {"erreur": "alpha Pareto invalide"}
        elif loi == "lognormale":
            mu = float(mu if mu is not None else np.log(max(seuil, 1)))
            sigma = float(sigma if sigma is not None else 1.0)
            if sigma <= 0:
                return {"erreur": "sigma lognormal invalide"}
        elif loi == "gpd":
            xi = float(xi if xi is not None else 0.0)
            beta = float(beta if beta is not None else max(seuil * 0.5, 1.0))
            if beta <= 0:
                return {"erreur": "beta GPD invalide"}
        else:
            return {"erreur": f"loi inconnue : {loi}"}

        rng = np.random.default_rng(int(st.session_state.get("seed_simulation", 42)))
        ...
        def generer(n):
            ...
            if loi == "pareto":
                ...
                return seuil * (U ** (-1.0 / alpha))
            if loi == "lognormale":
                return np.maximum(rng.lognormal(mu, sigma, size=n), seuil)
            if loi == "gpd":
                U = rng.uniform(size=n)
                if abs(xi) < 1e-10:
                    return seuil - beta * np.log(np.clip(U, 1e-12, 1.0))
                return seuil + beta / xi * ((1 - np.clip(U, 1e-12, 1-1e-12)) ** (-xi) - 1)
            return np.array([], dtype=float)

        resultats = []
        for t_info in prog:
            D = float(t_info["priorite"]); P = float(t_info["portee"])
            r = int(t_info.get("nb_reconstitutions", 0) or 0)
            aal = t_info.get("AAL"); aad = t_info.get("AAD")
            cap = (r + 1) * P
            def simuler(avec_aal=True, avec_aad=True, avec_rec=True):
                charges = []
                for _ in range(int(n_sim)):
                    N_s = rng.poisson(lambda_); S_total = 0.0
                    if N_s > 0:
                        Sp = generer(int(N_s))
                        idx_c = rng.choice(len(coeffs), size=int(N_s), replace=True)
                        for i in range(int(N_s)):
                            s = float(Sp[i]); c = float(coeffs[idx_c[i]])
                            if s <= D: S_i = 0.0
                            elif s <= D + P: S_i = c * (s - D)
                            else: S_i = c * P
                            S_total += S_i
                    ch = S_total
                    if avec_aad and aad: ch = max(ch - float(aad), 0.0)
                    if avec_aal and aal: ch = min(ch, float(aal))
                    charges.append(min(ch, cap) if avec_rec else ch)
                return np.array(charges)
            def calc_taux(ch):
                P0 = np.mean(ch); sig = np.std(ch)
                tp = P0 / gnpi_v; tr = (P0 + 0.2 * sig) / gnpi_v
                tt = tr / max(1 - t_info["brokage"] - t_info["frais"] - t_info["marge"] - t_info["retrocession"], 0.01)
                return round(tp,6), round(tr,6), round(tt,6), round(sig / max(gnpi_v, 1.0), 6)
            c_base = simuler(True, True, True)
            tp, tr, tt, sig = calc_taux(c_base)
            _, _, tt_aal, _ = calc_taux(simuler(False, True, True))
            _, _, tt_aad, _ = calc_taux(simuler(True, False, True))
            _, _, tt_rec, _ = calc_taux(simuler(True, True, False))
            resultats.append({
                "tranche": t_info["nom"], "type": t_info["type"],
                "taux_pur": tp, "taux_risque": tr, "taux_technique": tt,
                "sigma_sim": sig,
                ...
                "parametres_simulation": {
                    "loi": loi, "alpha": alpha if loi == "pareto" else None,
                    "mu": mu if loi == "lognormale" else None,
                    "sigma": sigma if loi == "lognormale" else None,
                    "xi": xi if loi == "gpd" else None,
                    "beta": beta if loi == "gpd" else None,
                    "lambda": lambda_, "seuil": seuil, "n_sim": int(n_sim),
                },
            })
        st.session_state["resultats_sim"] = resultats
        return _json_safe({"status": "ok", "parametres": resultats[0].get("parametres_simulation", {}) if resultats else {}, "resultats": resultats})
    ...
    def executer_outil_full(nom, inputs, f_tri_f, f_gnp_f, f_idx_f, f_mkt_f, gnpi_v, annee_v):
        ...
        if nom == "analyser_et_transformer_triangle":
            ...
            prog = st.session_state.get("tranches_p3", tranches_input)
            return _executer_simulation_loi_p3(
                loi=inputs.get("loi", st.session_state.get("loi_sim", "pareto")),
                ...
                mu=inputs.get("mu", st.session_state.get("loi_mu")),
                sigma=inputs.get("sigma", st.session_state.get("loi_sigma")),
                xi=inputs.get("xi", st.session_state.get("gpd_xi")),
                beta=inputs.get("beta", st.session_state.get("gpd_beta")),
...
"""
    ...
    def run_agent_full(api_key, f_tri_f, f_gnp_f, f_idx_f, f_mkt_f,
                       gnpi_v, annee_v, contexte_v, tarification_manuelle_v, seuil_al,
...
{tarification_manuelle_v if tarification_manuelle_v else "Aucune valeur externe fournie."}

═══ RÈGLES ABSOLUES ANTI-HALLUCINATION ══════════════════════
1. Ne jamais inventer de taux, prime, paramètre, p-value, R², sinistre majeur ou score.
2. Le tarif officiel provient du moteur actuariel ou de la tarification manuelle externe ; le LLM ne le crée pas.
3. Les valeurs externes R/Excel/SAS doivent être citées séparément et comparées sans modification.
4. Si une donnée manque, écrire : données insuffisantes pour conclure.
5. Les programmes proposés doivent rester comparables au programme initial et servir la négociation du réassureur leader.
6. Les propositions doivent viser la stabilité, la minimisation de la variance et la convergence des méthodes, pas un avantage explicite pour une partie.
...
5. SIMULATION (loi retenue : Pareto, Lognormale ou GPD ; utiliser ses paramètres validés)
...
9. PROPOSITION de programmes alternatifs comparables + rapport d’aide à la négociation
...
Agis de façon professionnelle et rigoureuse. Ne prétends jamais que le LLM garantit un tarif ; il aide à l’interprétation et à la structuration des propositions."""
        ...
        with result_cont:
            st.markdown("---")
            st.markdown("##  Résultats de l’assistant d’interprétation")
    ...
    if lancer3:
        st.markdown("---")
        st.markdown("###  Exécution de l’assistant d’interprétation")
        ...
        with log_cont:
            st.markdown("""<div style="background:linear-gradient(135deg,#0d0d1a,#1a1a1a);
                ...
                <span style="color:#3b82f6;font-weight:700"> Assistant d’interprétation démarré</span>
                <span style="color:#888;font-size:12px;margin-left:8px">
                Traitement en cours ...</span>
                </div>""", unsafe_allow_html=True)
        try:
            run_agent_full(api_key, f3_tri, f3_gnp, f3_idx, f3_mkt,
                           gnpi3, annee3, contexte3, tarification_manuelle3, seuil_alerte,
                           log_cont, result_cont, alert_cont)
            with log_cont:
                st.success(" Assistant d’interprétation terminé — rapport disponible ci-dessus et dans tous les onglets")
        except Exception as e:
        ...
        if st.button("Relancer l’assistant d’interprétation", key="relancer3"):
...
with tab_hist:
    ...
    if not user_email_hist:
        ...
        if not sessions:
            ...
            # Suppression
            st.divider()
            st.markdown("###  Supprimer une session")
            with st.expander("Supprimer (irréversible)", expanded=False):
                ...
                del_choix = st.selectbox("Session à supprimer", list(del_options.keys()),
                                          key="hist_del_select")
                if st.button(" Confirmer la suppression", key="hist_del_btn"):
                    try:
